...
class PeftModelFromCLIP_context(nn.Module):
    def __init__(self, cfg, clip_model, num_classes):
        super().__init__()

        #self.text_encoder = CLIP_Text(clip_model)
        #self.image_encoder = Peft_ViT(clip_model)
        self.image_encoder = clip_model
        self.tuner1 = ViT_Tuner(cfg, clip_model, num_classes)
        self.tuner2 = ViT_Tuner(cfg, clip_model, num_classes)
        self.tuner3 = ViT_Tuner(cfg, clip_model, num_classes)

        #feat_dim = self.image_encoder.out_dim
        feat_dim = 1536

        dtype = self.image_encoder.patch_embed.proj.weight.dtype
        #dtype = self.image_encoder.dtype
        ...
        self.head3 = eval(cfg.classifier)(feat_dim, num_classes, dtype, **cfg)
    ...
    def forward(self, image1,image2,image3, use_tuner=True, return_feature=True):
        #if (return_feature) and (not use_tuner):
        #    return (self.image_encoder.forward_features(image1, self.tuner1), self.image_encoder.forward_features(image2, self.tuner2)
        #            , self.image_encoder.forward_features(image3, self.tuner3))
        if use_tuner:
            return self.image_encoder(image1, self.tuner1), self.image_encoder(image2, self.tuner2), self.image_encoder(
                image3, self.tuner3)
        else:
            return self.image_encoder(image1), self.image_encoder(image2), self.image_encoder(
                image3)
        #head = self.head if not return_feature else None
        #return self.image_encoder(image, tuner, head)
